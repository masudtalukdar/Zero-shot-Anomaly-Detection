...
def tokenize(texts: Union[str, List[str]], context_length: int = 77, truncate: bool = False) -> Union[torch.IntTensor, torch.LongTensor]:
    ...
    sot_token = _tokenizer.encoder.get("<|startoftext|>", 0)
    eot_token = _tokenizer.encoder.get("<|endoftext|>", 0)
    all_tokens = []
    for text in texts:
        tokens = [sot_token]
        for tok in _tokenizer.encode(text):
            try:
...
class AnomalyCLIP_PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details):
        ...
        self.device = next(clip_model.parameters()).device  # Use the model's device
        ...
        if ctx_init_pos and ctx_init_neg:
            ...
            if True:
                ...
                ctx_vectors_pos = torch.empty(self.n_cls, self.normal_num, n_ctx_pos, ctx_dim, dtype=dtype).to(self.device)
                ctx_vectors_neg = torch.empty(self.n_cls, self.anormaly_num, n_ctx_neg, ctx_dim, dtype=dtype).to(self.device)
            else:
                print("Initializing a generic context")
                ctx_vectors_pos = torch.empty(n_ctx_pos, ctx_dim, dtype=dtype).to(self.device)
                ctx_vectors_neg = torch.empty(n_ctx_neg, ctx_dim, dtype=dtype).to(self.device)
        ...
        with torch.no_grad():
            ...
            embedding_pos = embedding_pos.reshape(normal_num, self.n_cls, l, d).permute(1, 0, 2, 3).to(self.device)
            embedding_neg = embedding_neg.reshape(anormaly_num, self.n_cls, l, d).permute(1, 0, 2, 3).to(self.device)
        ...
        tokenized_prompts_pos = tokenized_prompts_pos.reshape(normal_num, self.n_cls, d).permute(1, 0, 2).to(self.device)

        n, d = tokenized_prompts_neg.shape
        tokenized_prompts_neg = tokenized_prompts_neg.reshape(anormaly_num, self.n_cls, d).permute(1, 0, 2).to(self.device)
    ...
    def forward(self, cls_id=None):
        ctx_pos = self.ctx_pos.to(self.device)
        ctx_neg = self.ctx_neg.to(self.device)

        prefix_pos = self.token_prefix_pos.to(self.device)
        prefix_neg = self.token_prefix_neg.to(self.device)
        suffix_pos = self.token_suffix_pos.to(self.device)
        suffix_neg = self.token_suffix_neg.to(self.device)
