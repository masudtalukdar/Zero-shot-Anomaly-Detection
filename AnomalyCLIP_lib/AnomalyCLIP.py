...
from .CLIP import ModifiedResNet
...
class AnomalyCLIP(nn.Module):
    ...
    def encode_text_learn(self, prompts, tokenized_prompts, deep_compound_prompts_text=None, normalize: bool = False):
        ...
        # Move prompts to the same device as the model
        prompts = prompts.to(self.positional_embedding.device)

        # Ensure positional_embedding is on the correct device
        ...
        if deep_compound_prompts_text is None:
            ...
            # Ensure deep_compound_prompts_text is on the correct device
            deep_compound_prompts_text = [t.to(x.device) for t in deep_compound_prompts_text]
            x = self.transformer([x, deep_compound_prompts_text, 0])

        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)  # [batch_size, n_ctx, transformer.width]

        # Ensure tokenized_prompts is on the correct device
