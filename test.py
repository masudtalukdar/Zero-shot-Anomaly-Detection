...
from AnomalyCLIP_lib import model_load
...
def test(args):
    ...
    if compound_prompts_text is not None:
        compound_prompts_text = [t.to(device) for t in compound_prompts_text]
    ...
    for idx, items in enumerate(tqdm(test_dataloader)):
        ...
        with torch.no_grad():
            ...
            anomaly_map = torch.stack([torch.from_numpy(gaussian_filter(i.cpu().numpy(), sigma=args.sigma)) for i in anomaly_map.detach()], dim=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--checkpoint_path", type=str, default='checkpoint/checkpoint.pth', help='path to checkpoint')
